...
final class HttpProcessor
    ...
    /**
     * stopped 表示HttpProcessor 实例是否已经被连接器终止来确认 process 是否也 应该停止
    ...
    /**
     * 提供processor一个socket
     * 1.如果available为ture,等待processor获取到上一个socket，等待通知
     * 2.如果available为false,接收socket,修改available为true，唤醒await()，让自己的线程接收处理socket
     *
    ...
    /**
     * 等待新分配的socket,一个processor只可以一个socket,加synchronized
     *  1.available为false,等待Connector提供一个新的socket
     *  2.available为true，修改available为false，并唤醒assign().告诉Connector，我们接收了socket
     *  3.把接收的socket赋值给一个新的socket引用返回,开始处理请求
     *
     *  为什么 await 需要使用一个本地变量(socket)而不是返回实例的 socket 变量呢？
     *  因为这样 一来，在当前 socket 被完全处理之前，实例的 socket 变量可以赋给下一个前来的 socket
     *
    ...
    private synchronized Socket await() {
        ...
        // available为false,等待Connector提供一个新的socket
    ...
    private void parseHeaders(SocketInputStream input)
        ...
        while (true) {
            ...
            // 如果header的名称和值都是空，则返回，说明已经无header可取，跳出循环
            // 如果header名称为空，值不为空，则抛出异常
            if (header.nameEnd == 0) {
    ...
    /**
     * 解析来的http请求，并且设置相应的request的属性
     * 1.解析请求来的http请求行，并放入requestLine
    ...
    /**
     * 处理来的http请求，当发生异常时必须被处理
     * 1.获取输入流SocketInputStream
     * 2.如果连接是持久的，没有抛出异常,并且状态不是shutdown,则循环的从socket输入流中读取,
     *   直到 HttpProcessor 被停止，一个异常被 抛出或者连接给关闭为止
     * 3.设置初始化reqeust,response
     * 4.如果没有发生异常，开始解析请求
     * 5.
    ...
    private void process(Socket socket) {
        // 表示处理过程中是否发生错误
        boolean ok = true;
        // 指代 Response 接口中的 finishResponse 方法是否应该被调用
        boolean finishResponse = true;
        SocketInputStream input = null;
        OutputStream output = null;

        //  从连接器 获得的缓冲 区大小，而不是从 HttpProcessor 的本地变量获得。
        //  这是因为对于默认连接器的用户而言，HttpProcessor 是不可访问的。
        //  通过传递 Connector 接口的缓冲区大小，这就使得使用连接器的任何人都可以设置缓冲大小
        ...
        while (!stopped && ok && keepAlive) {
            // 修改finishResponse标志为true
            finishResponse = true;
            // 设置初始化request和response
            try {
            ...
            try {
                if (ok) {
                    // parseConnection方法获得协议的值
                    parseConnection(socket);
                    // 解析请求，设置request对应的属性值
            ...
            try {
                ...
                if (ok) {
                    // 如果ok为true，从connector获取container处理我们的请求
            ...
            // 如果finishResponse 仍然是 true
            // 响应对象的 finishResponse 方法和请求对象的 finishRequest 方法将被调用，并且结束输出。
            ...
            // 查响应的 Connection 头部是否已经在 servlet 内部设为 close
            // 或者协议是 HTTP1.0.如果是这种情况的话，keepAlive 设置为 false
            ...
            // 重置request和response,请求和响应对象接 着会被回收利用
